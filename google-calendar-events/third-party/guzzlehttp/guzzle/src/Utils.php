...
EOT
    ...
    public static function normalizeIdnConversionOption($value): ?int
    {
        ...
        if (\is_string($value) && \is_numeric($value) || \is_float($value) && \is_finite($value)) {
            \SimpleCalendar\plugin_deps\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing %s as the "idn_conversion" request option is deprecated; guzzlehttp/guzzle 8.0 will reject values that are not true, false, null, or an integer IDNA_* bitmask.', self::describeType($value));
            return (int) $value;
        }
