...
class Client implements ClientInterface, \SimpleCalendar\plugin_deps\Psr\Http\Client\ClientInterface
{
    ...
    {
        \SimpleCalendar\plugin_deps\trigger_deprecation('guzzlehttp/guzzle', '7.1', '%s::%s() is deprecated and will be removed in 8.0.', __CLASS__, __FUNCTION__);
        if (\count($args) < 1) {
    ...
    {
        \SimpleCalendar\plugin_deps\trigger_deprecation('guzzlehttp/guzzle', $since, 'Passing %s to request option "%s" is deprecated; guzzlehttp/guzzle 8.0 requires %s.', \get_debug_type($value), $option, $expected);
    }
    /**
        ...
        if (!empty($options['auth']) && \is_array($options['auth'])) {
            ...
            switch ($type) {
                ...
                case 'digest':
                    ...
                    break;
                case 'ntlm':
                    \SimpleCalendar\plugin_deps\trigger_deprecation('guzzlehttp/guzzle', '7.12', 'Passing "ntlm" as the built-in auth type is deprecated; guzzlehttp/guzzle 8.0 will no longer apply NTLM through the "auth" request option. NTLM is also deprecated by curl/libcurl and may be unavailable in current or future libcurl builds. Avoid NTLM; if you must use it temporarily, configure cURL HTTP authentication options directly with a libcurl build that still supports NTLM.');
